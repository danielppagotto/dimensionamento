...
# Calculando a demanda futura com base no status de saúde 

```{r eval=FALSE, include=FALSE}

#SELECT SG_UF, 
#            SUM(BENEF_ASSISTENCIA_MEDICA) AS BENEF_MEDICO,
#            SUM(POPULACAO) AS POP       
#FROM Dados.ans_cobertura."taxa_cobertura.parquet"
#WHERE PERIODO = 2022 AND CD_UF = 52
#GROUP BY SG_UF

```

Vamos fazer as estimativas apenas para a população SUS dependente. Para isso, vamos utilizar o valor de 80% por ser o valor da população SUS dependente no estado de Goiás como um todo em 2022 de acordo com dados da ANS. 

Além disso, já vamos calcular o total de gestantes de alto risco e risco habitual considerando alguns parâmetros nacionais estabelecidos no caderno de critérios e parâmetros. 

```{r}

previsoes <- previsoes %>% 
  mutate(nasc_SUS = total_nasc * 0.80,
         nasc_sus_hab = nasc_SUS * 0.85,
         nasc_sus_risc = nasc_SUS * 0.15)

```

Existem procedimentos que são realizados para todas as gestantes, assim como os procedimentos que são realizados apenas para as de alto risco. Além disso, existem percentuais para parto cesáreo e parto normal. Aproximadamente, de acordo com o SINASC, 58% dos partos de Goiás são cesáreos. 

```{r}
# frequencia de procedimento 

f_procedimentos <- previsoes %>% 
  mutate(prenatal = nasc_SUS * 6,
         acoes_educa = nasc_SUS * 0.4,
         consulta_espec = nasc_sus_risc * 5,
         puerperio = nasc_SUS * 4,
         parto_ces = nasc_SUS * 0.58, # parametro ajustavel
         parto_nor = nasc_SUS * 0.42, # parametro ajustavel
         ultrasson_risc = nasc_sus_risc * 2,
         ultrasson_hab = nasc_sus_hab * 1)


```

Agora vamos calcular a demanda considerando os tempos de procedimentos
...
t_procedimentos <- 
  f_procedimentos %>% 
  mutate(t_prenatal = prenatal * 0.5,
         t_educativas = acoes_educa * 1,
         t_consulta_espec = consulta_espec * 0.5,
         t_puerperio = puerperio * 0.5,
         t_parto_ces = parto_ces * 4.4,
         t_parto_nor = parto_nor * 2.53,
         t_ultrasson_risc = ultrasson_risc * 0.5,
         t_ultrasson_hab = ultrasson_hab * 0.5)

```

Esses tempos se referem ao trabalho mensal. Alguns dos procedimentos listados são realizados por médicos e enfermeiros. Para o presente estudo, vamos assumir que o trabalho os procedimentos da APS são realizados 50% por médios e 50% por enfermeiros. Já os procedimentos da atenção especializada são realizados por médicos. Fazemos isso especialmente por dificuldade de comparar com a oferta do CNES-PF. 
...
Vamos selecionar apenas os procedimentos de competência dos médicos e enfermeiros, pois estes  realizados por médicos e enfermeiros.

Não vamos acessar procedimentos de natureza laboratorial, pois existem outras categorias profissionais envolvidas. 

```{r}

procedimentos <- read_excel("análise de procedimentos/tempos_procedimentos.xlsx") %>% 
                        filter(codigo_familia_cbo == '2235' |
                               codigo_familia_cbo == '2231') %>% 
                        filter(tipo_procedimento != 'Exame laboratorial')
...
# Calculando o percentual de participação de atividades na AE (ambulatorial)

Tal cálculo será utilizado na oferta 

```{r}

sia_medicos <- sqlQuery(channel, 'SELECT * FROM "Analytics Layer".Infraestrutura."Atuação médicos obstetras - SIA"')

sia_gineco <- sia_medicos %>% 
                  filter(CATEGORIA != "NA") 

sia_gestantes <- sia_gineco %>% 
                    filter(str_detect(CATEGORIA, 'gravidez')|
                           str_detect(CATEGORIA, 'parto')|
                           str_detect(CATEGORIA, 'gestação')|
                           str_detect(CATEGORIA, 'contraç$')|
                           str_detect(CATEGORIA, 'Hipertensão materna')|
                           str_detect(CATEGORIA, 'mãe')|
                           str_detect(CATEGORIA, 'aborto')|
                           str_detect(CATEGORIA, 'feto')|
                           str_detect(CATEGORIA, 'Placenta prévia')|
                           str_detect(CATEGORIA, 'recém-nascido')|
                           str_detect(CATEGORIA, 'Eclâmpsia')|
                           str_detect(CATEGORIA, 'pós-natal')|
                           str_detect(CATEGORIA, 'nascido'))

sum(sia_gestantes$QTD_PROCEDIMENTOS)/sum(sia_gineco$QTD_PROCEDIMENTOS)
