...
author: "Daniel Pagotto, Sheila Mara Pedroza, Erika Aquino"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
setwd("~/GitHub/dimensionamento/01_RN e CD infantil")
...
- **O presente documento é um artefato intermediário da metodologia e se encontra em desenvolvimento** 
- **O artefato é uma versão *baseline*, que compreende parte dos procedimentos necessários para atender a população alvo**
...
df1 <- sqlQuery(channel, 'SELECT * FROM "Analytics Layer"."Epidemiológico".Nascimentos."Nascidos Vivos por mês, ano e município"', 
                as.is = TRUE)

df1 <- df1 %>% 
          rename(mes_ano = ano) %>% 
          mutate(qtd = as.integer(qtd),
                 ano = substr(mes_ano, 3, 7),
                 ano = as.integer(ano)) %>% 
          mutate(CODMUNRES = as.integer(CODMUNRES))

df2 <- sqlQuery(channel, "
SELECT ID_CMPT, SUBSTR(ID_CMPT, 1, 4) AS ANO,
       SG_UF, CD_MUNICIP, SUM(CAST(NR_BENEF_T AS INT)) AS TOTAL_BENEF
FROM Dados.ans_municipios.tb_bb
WHERE SG_UF = 'SP' AND SUBSTR(ID_CMPT, 5, 2) = '06'
GROUP BY ID_CMPT, SG_UF, CD_MUNICIP")

```

Sabe-se que, ao nascer, o RN passará por consultas e pela triagem neonatal. 

Passados três anos, ela passará por uma consulta anual para acompanhar o crescimento.
...
Por fim, foram consultados dados sobre a prevalência de doenças em RN, diagnosticadas por meio dos testes feitos nos primeiros dias de vida. Levantamos essa informação, mas não incluímos na metodologa *baseline*.
...
# Estudo de caso - Goiás 

Vamos fazer uma análise apenas para as regiões de saúde de Goiás em 2020.
...
servicos_go <- 
  ...
  left_join(hierarquia_municipios, by = c("CODMUNRES"="cod_municipio")) %>% 
  filter(cod_uf == 52) %>% 
  group_by(cod_regsaud, regiao_saude_pad) %>% 
  summarise(qtd_liquido = sum(qtd_liquido),
            qtd_consultas = sum(qtd_consultas),
            qtd_triagem_fisica = sum(qtd_triagem_fisica),
            qtd_vacinas = sum(qtd_vacinas),
