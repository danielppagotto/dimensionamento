...
demanda <- df_sb0_14 %>% 
              filter(ano == 2020) %>% 
              mutate(curativos_horas_anual = (proced_curativos * (1/3)),
                     curativos_horas_mensal = curativos_horas_anual/12)


demanda %>% 
  select(curativos_horas, endodontia_horas) %>% 
  gather(key = "procedimento", value = "horas") %>% 
  ggplot(aes(x = procedimento, y = horas))
